        ...
        if (!formValidity.valid) {
          if (this.config.flashMessage) {
            this.config.flashMessage.flash(
              this.config.messages?.validationError || "",
              false
            );
          }
          this.#setState("error");
  ...
  async #submit() {
    ...
    if (this.config.send !== undefined) {
      ...
      if (res.success) {
        if (this.config.flashMessage) {
          this.config.flashMessage.flash(
            res.message || this.config.messages?.success || "",
            true
          );
        }
        ...
        if (this.config.flashMessage) {
          this.config.flashMessage.flash(
            res.message || this.config.messages?.validationError || "",
            false
          );
        }
    ...
    // default send function
      ...
      try {
        ...
        if (jsonRes.errors !== undefined) {
          ...
          if (this.config.flashMessage) {
            this.config.flashMessage.flash(
              jsonRes.message || this.config.messages?.validationError || "",
              false
            );
          }
          ...
          if (this.config.flashMessage) {
            this.config.flashMessage.flash(
              jsonRes.message || this.config.messages?.success || "",
              true
            );
          }
